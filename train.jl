...
labels = map(label -> label == 0 ? 10 : label, read(traininglabels, UInt8, labelscount))
...
for i = 1:10
  println("Initial cost for $(i): $(cost(initial_theta, X, map(label -> label == i ? 1 : 0, labels), 0.1))")
  ...
  matching_labels = map(label -> label == i ? 1 : 0, labels)
  @time optimized = optimize(
    theta -> cost(theta, X, matching_labels, 0.1),
    (theta, storage) -> storage[:] = gradient(theta, X, matching_labels, 0.1),
    initial_theta,
    LBFGS()
  )

  println("Optimized cost for $(i): $(Optim.minimum(optimized))")
  println(Optim.minimizer(optimized))
end
